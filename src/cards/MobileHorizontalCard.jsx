...
const MobileHorizontalCard = ({ id, name, poster, posterV, genre, plot, trailer, duration, director, stars }) => {
...
const Popover = ({ name, poster, posterV, genre, plot, trailer, duration, director, stars, onClose }) => {
    ...
    return (
        <>
            {/* Backdrop */}
            <div
                className="relative inset-0 bg-black/30 backdrop-blur-md z-0"
                onClick={handleClose}
            ></div>
            {/* Popover */}
            <div
                className={`absolute bottom-0 left-5 transform -translate-x-1/2 bg-white text-black rounded-t-2xl z-20 shadow-lg transition-transform duration-300 ease-in-out hide-scrollbar ${isClosing ? styles.animateSlideOut : styles.animateSlideIn
                    }`}
                style={{
                    width: "calc(100% - 32px)",
                    height:"650px",
...
export default MobileHorizontalCard;
